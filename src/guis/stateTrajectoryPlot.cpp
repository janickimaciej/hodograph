...
StateTrajectoryPlot::StateTrajectoryPlot(const Scene& scene, const glm::vec2& pos,
...
void StateTrajectoryPlot::update()
{
	...
	if (ImPlot::BeginPlot("State trajectory", {m_size.x - offset, m_size.y - offset}))
	{
		...
		ImPlot::PlotLine("", m_scene.getXVector(), m_scene.getVVector(),
			m_scene.getIterations() - 1);
		ImPlot::EndPlot();
	}
